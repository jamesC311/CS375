...
public class WordFrequencyAnalyzer {
	...
	/**
	 * This method executes the adding of words to WFC and the utilization of the WordCounter
	 */
	...
	private void outputAnalysis(){
		if(outputSource instanceof java.io.PrintStream){
			((java.io.PrintStream)outputSource).printf("%-40s\t%s\n%-40s\t%s", "Word", "Frequency","----","---------");
			for(int i = 0; i < words.size(); i++){
				((java.io.PrintStream)outputSource).print(words.get(i).toString()+"\n");
			}
		}
		if(outputSource instanceof javax.swing.JTextArea){
			for(int i = 0; i < words.size(); i++){
				((javax.swing.JTextArea)outputSource).append("  " + words.get(i).toString()+"\n");
			}
		}
	}
